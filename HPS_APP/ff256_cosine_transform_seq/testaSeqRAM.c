...
#define PORT_1_MEM_BASE 0x5a000
#define PORT_1_ADDR_SPAN 3
...
int main() 
{
	...
	printf("*---------------------------------------------------------------------\n");
	printf("* FUNCTION       : COMPUTATION LIGHWEIGHT FF256CT-I TEST\n");
	printf("* PROJECT        : FINITE FIELD GF256 COSINE TRANSFORMS\n");
	printf("* DATE           : 08/09/2024\n");
	printf("*---------------------------------------------------------------------\n");
	...
	if(entrada)
	{
		for(i = 0; i <= (PORT_1_ADDR_SPAN); i++)
		{
			...
			else
			{
				if(i == 1)
				{
					mem_write = 0x00000000;
				}
				else
				{
					mem_write = 0x00000001;
				}
			}
